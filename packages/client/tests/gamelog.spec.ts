 ...
 * Uses the pinned space_hulk_1 seed-43 win by orders (stage 4 step 5, 2026-09-13; the same path as win.spec) so the dialog
...
test('end dialog exports the gameplay log with notes, mission and timestamp filename', async ({ page }) => {
  ...
  await page.goto('/?deploy=0&tick=0&mission=space_hulk_1&seed=43');
  ...
  expect(log.meta.formatVersion).toBe(2); // 2 since the metered pause (stage 4 step 5)
  expect(log.meta.mission).toBe('space_hulk_1');
  expect(log.meta.seed).toBe(43);
