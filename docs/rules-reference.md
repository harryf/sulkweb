...
Every marine has a second slot, the **squad task**, written only by the squad planners. His behaviour reads the slots in a fixed order: his own order first, the squad task second, the default list last, so an individual order always beats the squad's plan. A direct-control key clears his own order and nothing else; the task waits out the lease and resumes. A squad order that takes effect clears the members' earlier individual orders (the squad command supersedes what was given before it). A task that cannot progress on a full AP pool (post unreachable, the column blocked) hands the tick to the parking rules, so the marine covers where he stands instead of standing idle; an individual order that makes no progress on a full pool for `orderStallTicks` (10) ticks is dropped.
