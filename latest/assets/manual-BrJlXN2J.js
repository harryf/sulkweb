...
disappears.</p>`},{id:"the-turn",title:"How the clock works",html:`
...
</ul>`},{id:"ap-pause",title:"Action points and command time",html:`
...
pause will not open at all.</p>`},{id:"moving",title:"Moving",html:`
...
bolters jam at the worst possible moment. The heavy flamer cannot overwatch.</p>`},{id:"flamer",title:"The heavy flamer",html:`
...
(the door key) at its centre.</p>`,t=16,d={corridor:"#3a3a40",room:"#2b3340",grid:"#17171c",door:"#d08030",entry:"#e04040",exit:"#40c040",deploy:"#5aa0e8",objective:"#e8c840",ducting:"#e8e840",cat:"#e8e8e8",download:"#40c8c8"};function v(e){return I[N[e]]}function S(e,n,r,a,x){const{dc:s,dr:i}=v(r),l=(e+1)*t+t/2,h=(n+1)*t+t/2,c=l-s*t*.35,w=h-i*t*.35,u=l+s*t*.3,f=h+i*t*.3,y=-i,T=s,D=`${u+y*t*.32},${f+T*t*.32}`,M=`${u-y*t*.32},${f-T*t*.32}`;return`<polygon class="${x}" points="${c},${w} ${D} ${M}" fill="${a}"/>`}function Y(e){const n=(e.width+2)*t,r=(e.height+2)*t,a=[];a.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${n} ${r}" role="img" aria-label="Map of ${e.name}">`),a.push(`<rect width="${n}" height="${r}" fill="#0d0d0d"/>`);for(const s of e.squares){const i=(s.x+1)*t,l=(s.y+1)*t;a.push(`<rect class="sq-${s.kind}" x="${i}" y="${l}" width="${t}" height="${t}" fill="${d[s.kind]}" stroke="${d.grid}" stroke-width="1"/>`)}for(const s of e.squares){if(!s.doorFacing)continue;const{dc:i,dr:l}=v(s.doorFacing),h=(s.x+1)*t+t/2,c=(s.y+1)*t+t/2,w=h+i*t/2,u=c+l*t/2,f=-l,y=i;a.push(`<line class="door" x1="${w-f*t*.45}" y1="${u-y*t*.45}" x2="${w+f*t*.45}" y2="${u+y*t*.45}" stroke="${d.door}" stroke-width="3"/>`)}for(const s of e.marineDeployment??[]){const i=(s.x+1)*t+t/2,l=(s.y+1)*t+t/2;a.push(`<circle class="deploy" cx="${i}" cy="${l}" r="${t*.3}" fill="${d.deploy}"/>`)}for(const s of e.entryPoints??[]){const i=s.facing??"up",{dc:l,dr:h}=v(i);a.push(S(s.x+l,s.y+h,i,d.entry,"entry"))}for(const s of e.exitPoints??[]){const i=s.facing??"up",{dc:l,dr:h}=v(i),c={up:"down",down:"up",left:"right",right:"left"};a.push(S(s.x+l,s.y+h,c[i],d.exit,"exit"))}const x=[...e.objectivePoint?[e.objectivePoint]:[],...e.objectivePoints??[],...e.downloadPoint?[e.downloadPoint]:[]];for(const s of x){const i=(s.x+1)*t+t/2,l=(s.y+1)*t+t/2,h=t*.38,c=e.downloadPoint===s?d.download:d.objective;a.push(`<polygon class="objective" points="${i},${l-h} ${i+h},${l} ${i},${l+h} ${i-h},${l}" fill="${c}"/>`)}for(const s of e.ductingSquares??[]){const i=(s.x+1)*t,l=(s.y+1)*t;a.push(`<rect class="ducting" x="${i+2}" y="${l+2}" width="${t-4}" height="${t-4}" fill="none" stroke="${d.ducting}" stroke-width="2"/>`)}if(e.catStart){const s=(e.catStart.x+1)*t+t/2,i=(e.catStart.y+1)*t+t/2;a.push(`<circle class="cat" cx="${s}" cy="${i}" r="${t*.25}" fill="none" stroke="${d.cat}" stroke-width="2"/>`)}return a.push("</svg>"),a.join("")}const W=[{swatch:`<svg viewBox="0 0 16 16"><rect x="1" y="1" width="14" height="14" fill="${d.corridor}"/></svg>`,label:"Corridor square"},{swatch:`<svg viewBox="0 0 16 16"><rect x="1" y="1" width="14" height="14" fill="${d.room}"/></svg>`,label:"Room square"},{swatch:`<svg viewBox="0 0 16 16"><line x1="2" y1="8" x2="14" y2="8" stroke="${d.door}" stroke-width="3"/></svg>`,label:"Door"},{swatch:`<svg viewBox="0 0 16 16"><polygon points="8,3 3,13 13,13" fill="${d.entry}"/></svg>`,label:"Genestealer entry point"},{swatch:`<svg viewBox="0 0 16 16"><polygon points="8,13 3,3 13,3" fill="${d.exit}"/></svg>`,label:"Marine exit"},{swatch:`<svg viewBox="0 0 16 16"><circle cx="8" cy="8" r="5" fill="${d.deploy}"/></svg>`,label:"Marine deployment square"},{swatch:`<svg viewBox="0 0 16 16"><polygon points="8,2 14,8 8,14 2,8" fill="${d.objective}"/></svg>`,label:"Objective square"},{swatch:`<svg viewBox="0 0 16 16"><rect x="3" y="3" width="10" height="10" fill="none" stroke="${d.ducting}" stroke-width="2"/></svg>`,label:"Ducting (mission 6)"},{swatch:`<svg viewBox="0 0 16 16"><circle cx="8" cy="8" r="4" fill="none" stroke="${d.cat}" stroke-width="2"/></svg>`,label:"C.A.T. start (Rescue)"}];function K(e){const r=e.slice(0,e.lastIndexOf("/")+1).replace(/\/$/,"").split("/").pop()??"";return r==="latest"||/^\d+\.\d+\.\d+(-alpha\.\d+)?$/.test(r)?"../versions.html":"versions.html"}const U={debug_1:"Kill everything, or reach the exit.",space_hulk_1:"Flame the Launch Control square. You lose the moment no living flamer has ammo.",space_hulk_2:"Kill 30 (blips count their hidden value), or blockade every entry point.",space_hulk_3:"Carry the C.A.T. out through an exit. Damaged = draw; destroyed = defeat.",space_hulk_4:"Burn both Gene Banks. You lose the moment no living flamer has ammo.",space_hulk_5:"Get 5 marines out through the exits. Fewer than 5 alive-plus-escaped = defeat.",space_hulk_6:"Survive to the end of turn 16. Lost ducting or a flamed control room = defeat.",beta_1:"Get any one marine out through the far exit.",beta_2:"Hold the Data Room with a sergeant for 4 quiet end phases. Both sergeants dead = defeat."},p=document.getElementById("manual");function o(e,n,r){const a=document.createElement(e);return n&&(a.className=n),r!==void 0&&(a.innerHTML=r),a}const b=o("header","manual-header");b.appendChild(o("h1",void 0,"SULK"));b.appendChild(o("p","manual-subtitle","Field manual: everything the ship will not tell you"));const A=o("a","back-link","&larr; Back to the game");A.href="./";A.id="back-to-game";b.appendChild(A);p.appendChild(b);const B=o("nav","manual-toc"),k=o("ul");for(const e of j)k.appendChild(o("li",void 0,`<a href="#${e.id}">${e.title}</a>`));k.appendChild(o("li",void 0,'<a href="#controls">Controls</a>'));k.appendChild(o("li",void 0,'<a href="#missions">The missions</a>'));B.appendChild(k);p.appendChild(B);const V=["what-is-this","ap-pause","shooting","flamer","doors","flames"];let P=0;for(const e of j){const n=o("section");if(n.id=e.id,n.appendChild(o("h2",void 0,e.title)),n.insertAdjacentHTML("beforeend",e.html),p.appendChild(n),V.includes(e.id)&&P<q.length){const r=q[P++],a=o("blockquote","marine-quote");a.appendChild(o("p",void 0,`&ldquo;${r.text}&rdquo;`)),a.appendChild(o("footer",void 0,`&mdash; ${r.attribution}`)),p.appendChild(a)}}const g=o("section");g.id="controls";g.appendChild(o("h2",void 0,"Controls"));g.insertAdjacentHTML("beforeend",F);const $=o("div","key-rose");for(const e of H){const n=o("div","key-row");n.style.paddingLeft=`${e.offset*3}em`;for(const r of e.caps){const a=o("div",r.label?"keycap":"keycap unbound");a.appendChild(o("span","key-letter",r.key)),a.appendChild(o("span","key-label",r.label??"")),r.sub&&a.appendChild(o("span","key-sub",`(${r.sub})`)),n.appendChild(a)}$.appendChild(n)}const O=o("div","key-row specials");for(const e of R){const n=o("div","keycap wide");n.appendChild(o("span","key-letter",e.key)),n.appendChild(o("span","key-label",e.label??"")),O.appendChild(n)}$.appendChild(O);g.appendChild($);const _=o("ul","key-notes");for(const e of G)_.appendChild(o("li",void 0,e));g.appendChild(_);p.appendChild(g);const m=o("section");m.id="missions";m.appendChild(o("h2",void 0,"The missions"));m.appendChild(o("p",void 0,"Every map below is drawn from the same mission data the game plays: squares, doors, entry points, and objectives exactly where you will find them."));const C=o("div","map-legend");C.id="map-legend";for(const e of W){const n=o("span","legend-item");n.insertAdjacentHTML("beforeend",e.swatch),n.appendChild(o("span",void 0,e.label)),C.appendChild(n)}m.appendChild(C);const Q=[...L,{key:"debug_1",name:E.debug_1.name,tagline:"The training scenario: one marine, a trickle of blips. Not on the mission select; start it with ?mission=debug_1.",squad:"1 storm bolter"}];for(const e of Q){const n=o("article","mission-entry");n.id=`mission-${e.key}`,n.appendChild(o("h3",void 0,e.name)),n.appendChild(o("p","mission-tagline",e.tagline));const r=o("ul","mission-facts");r.appendChild(o("li",void 0,`<strong>Objective:</strong> ${U[e.key]??""}`)),r.appendChild(o("li",void 0,`<strong>Squad:</strong> ${e.squad}`)),n.appendChild(r);const a=o("div","mission-map");a.innerHTML=Y(E[e.key]),n.appendChild(a),m.appendChild(n)}p.appendChild(m);const X=o("footer","manual-footer",`A fan recreation of Toby Woodwark&rsquo;s <em>Sulk</em> (2003) &middot; <a href="https://www.gnu.org/licenses/gpl-3.0.html">GPL-3.0</a> &middot; Space Hulk is a trademark of Games Workshop; this project is unaffiliated and claims no rights over Games Workshop&rsquo;s intellectual property. &middot; <a id="credits-link" href="credits.html">Audio credits</a> &middot; <a id="versions-link" href="${K(location.pathname)}">All versions</a> &middot; <span id="app-version">latest-59df4fe</span>`);p.appendChild(X);
