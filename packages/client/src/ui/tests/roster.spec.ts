...
describe('RosterPanel DOM', () => {
  ...
  it('facing arrow, AP, and own-line ammo render and stay live (ISC-288/289/290)', () => {
    panel = new RosterPanel(buildRoster(engine, engine.mission), stats, () => {})
    ...
    expect(card.querySelector('.m-stats')!.textContent).toBe('AP 3/4') // turn cost 1 AP
    ...
    expect(fCard.querySelector('.m-stats')!.textContent).toBe('AP 4/4') // no ammo in the stats line…
