...
| `scripts/fetchAudio.ts` | Downloads/cuts the gitignored audio, driven by `audioManifest.ts` |
...
- Every audio asset flows through `audioManifest.ts` (what it is, where it came from, who to credit); `AudioManager.queueLoads` loads it and an event handler plays it. Committed original wavs live in `assets/sounds/`; fetched/generated audio lives in gitignored `assets/audio/` and is produced by `scripts/fetchAudio.ts`. The gitignore has deployment consequences, covered in [architecture.md](architecture.md) under Deployment. New audio needs a credit entry; `CREDITS.md` is generated from the manifest data.
