...
## Stage 3 as built (2026-09-13)

Shipped as v2.0.0-alpha.3 (prerelease, its own frozen directory; the root stays v1.1.0). Built without the two playtest answers from "Next session" (Harry said "implement stage 3"), so the shipped tuning (regen.marine 3) and the /latest/ facing fix ride along. Read this before stage 4.

### What the build changed from the sketches

- The squad slot is a second order slot on the marine, `Piece.task`, written only by the squad planners (ai/squad.ts); the executor in ai/orders.ts reads the player's order first, the task second, so "individual beats squad" is structure, not a priority number. The planner is the task's single writer: completion never clears a task (the executor keeps reporting done and holds the marine to his post facing), so a re-plan that changes nothing emits nothing and the marker stays on the post. The SystemsThinking pass found the pump the other design would have had (executor clears, planner rewrites, marker blinks every trigger).
- Squad commands ride the one command path, addressed to any member (the squad is his deployment tag): the log gains squadOrder and clearSquadOrder and the squadOrderChanged event, format version unchanged.
- The relay is a due tick, not a per-marine delivery: issued + 1 with a living sergeant, issued + TUNING.relayTicks (8) without, and the coordination flag is fixed at issue, so a sergeant killed mid-order degrades the next order, not the live one (the plan left this open; the advisor's reading).
- A squad order that takes effect clears the members' earlier player orders (the advisor's deaf-marine case: a marine still walking an old moveTo while his squad forms up reads as a bug). Player orders given after it still win.
- A player's order that makes no progress on a full pool for TUNING.orderStallTicks (10) is dropped; a squad task that cannot progress on a full pool lets the default list run (he covers where he stands), and an overwatching marine keeps his overwatch while his march is blocked (the first build dropped it every tick).
- Direct control pins: a member the player steered within the last cycle keeps his square and facing as his defend post and is left alone by the other planners, so the player's placement is not undone the moment the lease ends (the SystemsThinking tug-of-war loop).
- Defend assigns in two steps: the post set by greedy set cover (an entrance square weighs four lane squares; a post already held scores one extra, the hysteresis against the two-equal-posts oscillation), then members to posts deepest-first by walk distance with the cannon on the longest lane. The first probe assigned by member order and sent the rear man of a column to the far door, with the corridor plugged by his own squad. Re-plan triggers are a member death, the cycle boundary and a pin change; the entrance door toggle trigger from the sketch was dropped (posts score with doors peeked open, so a toggle carries no information and the hive owns a re-plan lever otherwise). Every square in the missions carries a section, so "clicked square plus radius 3" only fires on section-less fixture boards.
- Advance fixes the column at the first plan (distance to the target, battle order as the tie-break); the first probe re-sorted every tick and the column flipped as it moved. The flamer is demoted once someone can pass him, never promoted, and leads while nobody can (a one-wide corridor cannot swap its head). Followers walk to their predecessor's square (adjacency completes) and the task always names his current square, so a stale target never walks a follower backward. The leader waits for the column to close up unless he is the plug (a follower with no path to his predecessor). The "one marine moves per tick" rule went: with persistent follow tasks it cannot be enforced without rewriting tasks every tick, and the leapfrog falls out of the gaps anyway. Contact is a threat in sight within TUNING.contactRange (6) of a member, and the suspension (TUNING.contactHoldTicks 8, leader only) re-arms only while it closes in, so a parked blip is terrain (the sketch's "any visible stealer for a cycle" stalled the column at the first blip in the SystemsThinking trace).
- Clear: the marine nearest the near flank opens whatever his type (a column's head in a corridor is the flamer, and the flamer is who wants that door open); covers are geometric (bodies ignored, since the opener will stand in the doorway) and required = min(2, bolters left, feasible posts); the opener goes when the covers stand on overwatch or TUNING.clearTimeoutTicks (20) after their tasks were issued (the advisor's stall guard); the flamer waits one square behind the opener when he can get there.
- The command pause is a scene flag (Space): the clock stops, right-click orders, Tab and Esc work, direct keys wait. Esc with a squad selected is hold (order and selection go); Esc otherwise still pauses.
- Engine 480 tests (squad.spec 40), client units 92, e2e see the ISA run; the pinned seeds (space_hulk_1 26, space_hulk_2 29) held without a re-scan because the autopilot issues no squad orders, which also means neither the seeds nor the scan exercise stage 3: a scripted squad-order sweep (scratch script, 48 games, sergeant-death variant included) checked replay determinism and liveness before the tag (numbers in the ISA Decisions). Known gaps at the tag: no human playtest of squad orders; the Interceptor real-Chrome check still blocked (headless Chromium stood in).
...
Built before any number was touched, per the handover. The autopilot has a second policy, `squads` (ai/SquadAutopilot.ts; `autoplay(engine, cycles, 'squads')`), that gives each squad the order a sensible player would give, one per change of state, and leaves the marines to the stage 3 planners: defend the section the squad stands in on the first call; advance toward the mission target once nothing has been in sight within contact range for 16 ticks (a contact restarts the count; a contact while advancing changes nothing, the advance planner already holds the leader, and a defend issued on contact would march the squad off overwatch to new posts under fire); clear a closed door on the leader's next step before the advance is issued and again on the way, at most twice per door, after which the column opens it on the march; defend the target's section on arrival; re-issue the advance when the leader has made no progress for 48 quiet ticks. The target per objective: the threshold square for the flame missions (the last square outside the objective's section on the way, so the squad never walks into the room it means to burn, and it holds there without a defend while the flamer's shot is pending), the entry post the blockade assigns the first member for kill-quota, the nearest exit for the missions that leave, the Data Room square for download, none for defend. The individual pass under this policy issues direct actions only: the flame (never with a squad-mate in the target section; the bot's flame does not pin him), the firing door, an approach order to the nearest firing square, the download sergeant's walk, the cat fetcher. The bot's constants live in `AUTOPILOT`, not TUNING: the scan compares tunings against a fixed policy. The default policy is byte-identical; the pinned seeds hold.

The instrument is `packages/engine/scripts/scan.ts` (flags: --missions, --seeds N or A-B, --policy orders|squads|both, --cycles, --tuning key:value, --help): per mission and policy the wins, losses, draws and games at the cap, the win rate with a 95% Wilson interval, the mean end cycle and living marines, the winning seeds and a determinism check of the first seed. Two or three wins in sixty cannot rank one change against another, which the interval makes visible.

Four stage 3 planner faults surfaced on the first quiet traces and are fixed (specs in squad_autopilot.spec and squad.spec): the advance column deadlocked in a corner pocket when the leader's waypoint was held by his own follower (a re-sort by distance, the one exception to the fixed column); the heavy flamer, demoted behind the column in a one-wide corridor, could never reach the door he had to burn through (on a flame mission with his job pending the column keeps him at its head); a clear posted its covers up to eight squares from the door, on the far side of a corridor loop, and split the squad (covers stand within three squares of the near flank); an advance issued over a live defend inherited the flamer's defend post as his march and the column never moved (a different squad order clears the members' tasks; the same order again is a re-plan and keeps the posts).

### The numbers (60 seeds, cycle cap 60, shipped tuning, 2026-09-13; both policies in one run after the planner fixes, raw output in docs/scans/2026-09-13-stage4-step1.txt)

| Mission | Policy | W | L | Win rate, 95% interval | Mean end cycle | Marines alive at the end | Winning seeds |
|---|---|---|---|---|---|---|---|
| space_hulk_1 | orders | 2 | 58 | 3.3% [0.9, 11.4] | 2.0 | 2.83 | 26, 27 |
| space_hulk_1 | squads | 2 | 58 | 3.3% [0.9, 11.4] | 2.1 | 2.17 | 24, 31 |
| space_hulk_2 | orders | 3 | 57 | 5.0% [1.7, 13.7] | 5.2 | 0.23 | 29, 36, 58 |
| space_hulk_2 | squads | 0 | 60 | 0.0% [0.0, 6.0] | 4.2 | 0.00 | none |

All nine missions under `squads`, ten seeds each: space_hulk_6 (defend) 10 of 10, space_hulk_4 1 of 10, every other mission 0 of 10; every game resolves, every row replays.

### The reading

1. No detectable difference at sixty seeds. The squad-orders game, played sensibly by a bot, shows the same two wins in sixty on space_hulk_1 as individual orders (on different seeds) and 0 against 3 on space_hulk_2; the intervals overlap on both missions, 0 of 60 against 3 of 60 is not a significant gap, and sixty seeds cannot see an effect of about ten points either way. What the table does say: squad orders alone did not make the bot's game easier, and the mean marines standing at the end fell (2.17 against 2.83 on space_hulk_1; a mean, no spread printed yet).
2. A hypothesis from the traces, not a result: space_hulk_1 dies in transit. Seed 26 wins under individual orders, whose cover rule stops every bolter on overwatch whenever an enemy is within ten squares; under the squad advance the leader alone holds on contact while the followers keep closing up, and on that seed the squad is wiped between ticks 37 and 50 walking down the first corridor. The test of the hypothesis is the fifth option below.
3. A second hypothesis: space_hulk_2's zero is the issuer's plan, not the game. The squad marches to one entry post as a column where the individual bot spreads across the entries; defending the start section all game instead lasts twice as long (mean end cycle 8) and still loses 60 of 60. A blockade wants marines split across entries, which is a mission order (build step 4), not a squad order. The nine-mission rows are ten seeds each: zero of ten still allows up to 28%, so they are liveness evidence, not balance.
4. Nothing was tuned. regen.marine stays 3; no TUNING value changed.

### For Harry, with the numbers beside them

- The transit rule (build step 2), the four options from the handover: overwatch kept while moving in formation; posting in stages, the nearest post first and the rest once it is covered; the order held until every post is reached; or accept the gap and let the issuer (and the player) sequence around it. The traces point at the gap as the source of the first corridor wipe (a hypothesis, see the reading); the individual bot's "stop and cover when the enemy is within ten squares" is the rule the squad advance lacks. A fifth option, which is also the test of that hypothesis: give the advance that rule (every follower on overwatch while a threat is within contact range, not only the leader) and re-scan the same sixty seeds before choosing among the four.
...
Ships on /latest/ (a user-facing rules change on the prerelease line) with space_hulk_2's zero named as the known gap (a squad marches to one entry post; build step 4); v2.0.0-alpha.4 will freeze it with the next steps. Next: build order step 4 (Harry's call), step 3 deferred behind it.

## Stage 4 step 4 entry notes (2026-09-13)

Harry, after step 2: "write any notes you need to so we can continue with step 4". So step 4 (mission orders) comes next and step 3 (all nine missions under the tick rules, a fixture per victory and loss path) is deferred behind it. The two step 2 refinements (staging under contact only, the executable plan) were not overruled and stand. The win-rate band per mission (step 6) is still owed.

### What a mission order is

The plan's L3 ("Mission orders (stage 4)" above): a squad order sent to every squad, plus one new order, `objective`, where each squad's target is the mission's own objective set. The squad state map and the three planners are per squad, so every mission order is a loop over `engine.squadNames()`; the relay is per squad too (a squad without a sergeant gets its order 8 ticks late and uncoordinated, as now). Recommended shape: one engine command, `missionOrder {order}`, addressed to any living marine, that fans out inside `execute` to one `squadOrder` per squad (one log entry, one `squadOrderChanged` per squad, replay-safe), rather than the client looping.

### What `objective` means per mission

`SquadAutopilot.squadTarget` already reads every objective at squad level and is the function to lift into the engine: the threshold square for the flame missions (the last square outside the room, the squad holds there while the flamer's shot is pending; `flameJobPending`), the nearest exit for escort, escape and exterminate-or-exit, the Data Room square for download (the sergeant's own walk follows), none for defend (objective = defend the section you stand in). The one that does not fit a single squad order is kill-quota, and the instrument said so twice: space_hulk_2 (one squad of five, eleven entries) loses 60 of 60 under any single squad order (advance to one post, or defend in place), while the individual-orders bot's per-marine blockade spread (`assignEntryPosts` in MarineAutopilot: each marine takes his nearest uncovered entry, standing there covers every entry within six walk squares, leftovers reinforce) wins 3 of 60. So `objective` on kill-quota needs a split, and the cleanest place is a fourth squad order, `blockade`: posts are the greedy entry cover from `assignEntryPosts` (per member, not per squad), written as level 2 tasks under the transit rules (armed walks, the hold on contact), re-planned on a death and at the cycle boundary like defend, `postsReached` when every post is held. `objective` then maps to blockade on kill-quota, defend on defend missions, and advance (with the flame hold) everywhere else. The other three mission orders are the existing ones fanned out with one target.

### Where it hooks

- Engine: `core/Commands.ts` (the `missionOrder` command and the `blockade` and `objective` kinds; `objective` is resolved to a concrete order per squad at receipt so the log stays concrete), `GameEngine.execute` (fan-out, `squadOrderIsValid` per squad), `ai/squad.ts` (`runBlockade`, sharing runDefend's staging and `postHeld`), `index.ts` exports.
- Bot: `SquadAutopilot` swaps its per-squad advance for the mission order on the first quiet count (the issuer becomes: defend, then `objective`), and kill-quota stops being a one-post advance. Re-scan the same sixty seeds on both hulks (the step 2 table is the baseline: space_hulk_1 squads 10 of 60, space_hulk_2 squads 0 of 60) and the nine missions at ten seeds; space_hulk_2 above zero is the exit criterion for the step.
- Client: `LiveScene.handleSquadOrderClick` is the squad path (right-click defend, Shift advance, door clear, Esc hold); a mission order needs one selection state for "all squads" (Tab cycling past the last squad to an all-squads selection is the natural key, the roster highlighting every row) and a key for `objective` (no key is assigned in the plan's Input summary; the free letters are listed in keyboardHelp.ts SPECIAL_KEYS). Markers: the squad markers per squad as now, one per squad. The roster word per squad row as now.
- Specs: `squad_autopilot.spec`'s target table moves to the objective resolver; a `mission.spec` for the fan-out (one command, N squadOrderChanged, per-squad latency), `blockade` (posts cover every entry within six walk squares, the hold on contact, re-plan on death), `objective` per mission; the client e2e for the all-squads selection and the key.

### Step 3, deferred

When it comes: a fixture per victory and loss path under the tick rules (defend's turn limit at cycle 16, download's cycles on the square, escort's cat carried to an exit, kill quota's count and the blockade check per cycle, flame objectives all cleansed and the loss when no flamer has ammo, escape count, exterminate-or-exit), plus the bot's row per mission as the liveness check. The nine-mission scan rows are the current state of that (step 2 table).

### Gotchas for the session

A flame mission is lost at the first victory check without a flamer with ammo, so fixtures need a loaded flamer; the transit turn lives in MarineAI's order branch, inside the formation flag; to isolate a planner rule, toggle it under the scan (one rule at a time on the ten-seed rows) before reading traces; `quiet()` fixtures (sh1 with blips zero), `inRoom()` and the corridor deploy are in squad.spec and transit.spec; the engine suite is 520 tests, e2e 124; pnpm build and the suites dirty packages/engine/tsconfig.tsbuildinfo (checkout it).

## Stage 4 step 4: mission orders (2026-09-13)

Built from the entry notes above; specs in mission_orders.spec (22 cases), squad_autopilot.spec and the client's mission.spec; the same sixty seeds re-scanned in one run, raw output in docs/scans/2026-09-13-stage4-step4.txt.

### As built

- **One command, every squad.** `missionOrder { order }` is addressed to any living marine and fanned out inside the engine to every squad with a living member: one log entry, one `squadOrderChanged` per squad, the relay per squad as before. The order is a `SquadOrderRequest`: a concrete squad order, or `objective`, resolved per squad at receipt (ai/objective.ts) into the order the mission wants of it: a blockade on kill-quota, a defend where its sergeant stands on defend missions, an advance to the threshold of the room to burn on the flame missions, to the Data Room square on download, to the nearest exit everywhere else. A squad with nothing left to do is skipped and the command is refused when no squad takes it; the log carries the request and the squad state holds the concrete order, so a replay resolves it the same way. `squadOrder` takes the same request, so one squad can be sent to the objective on its own. One refinement the systems pass forced: the receipt is idempotent per squad. A squad already on the resolved order and past its relay is left alone (no reset, no event), because a squad order that repeats the live order is a re-plan (the planner restarts, the relay runs again), and a fan-out that re-planned every squad each time one squad re-issued would have every squad restarting the others (the accidental adversaries loop: two local fixes degrading each other's plan).
- **The blockade.** A fourth squad order with no target square. Its posts are a greedy set cover of the mission's entries by squares under the victory metric itself (a marine within 6 raw-adjacency squares of an entry covers it; walls block, doors and bodies do not), one post per marine: each round takes the square covering the most entries still uncovered and gives it to the nearest free marine by walk, once every entry is covered the marines left take the best remaining squares by entries in reach (the reinforcement), and a post must be reachable from its marine without crossing a post already chosen (the first quiet run walled the last reinforcement off behind his own squad-mates' posts in the corridor). The cover is computed over every living marine the player has not taken and each squad runs its slice as level 2 tasks under the transit rules; it is sticky between re-plans (a death, a pin, the cycle boundary). A probe settled the plan's shape before the build: on space_hulk_2 two squares cover all eleven entries, one per corner cluster, so the blockade is two marines on the right squares and three reinforcing. Two refinements recorded for Harry to overrule: the blockade does not stage under contact (the advisor's case: the posts are one per marine and far apart by design, a hold in the deployment room covers nobody's post, and staging would keep four marines in their rooms while the cover is never complete at a boundary; the switch `BLOCKADE_STAGING` is kept for the instrument), and the bot's first call on kill-quota is the blockade itself rather than a gathering defend (space_hulk_2 deploys one marine per room; gathering into the sergeant's room and marching out again costs the cycles the blockade needs to hold).
- **The bot issues objective.** The first squad past its quiet gate (door ahead cleared first, as before) issues one `missionOrder { objective }` per game; every later re-issue (a stall, a cleansed target, a clear on the way) is that squad's own `squadOrder { objective }`, and a squad whose live order another squad's fan-out changed adopts it (phase and target from the engine's state) rather than issuing on top of it. The mission order is one change of the mission's state and gets one issuer.
- **Client.** Tab past the last squad selects every squad at once (two or more squads; every roster row lit, rings on every marine in his squad's colour); with every squad selected a right-click is a mission order (defend, Shift advance, a door edge clears) and Esc drops every squad's order; I sends the selected squad, or every squad, to the objective. A blockade draws no target marker (the posts' own rings carry it); the squad row reads BLOCKADE.

### The numbers (60 seeds, cycle cap 60, shipped tuning, 2026-09-13)

| Mission | Policy | Step 2 wins of 60 | Step 4 wins of 60 | Step 4 rate, 95% interval | Marines alive at the end (step 2 to step 4) | Step 4 winning seeds |
|---|---|---|---|---|---|---|
| space_hulk_1 | orders | 2 | 2 | 3.3% [0.9, 11.4] | 2.83 to 2.83 | 26, 27 |
| space_hulk_1 | squads | 10 | 10 | 16.7% [9.3, 28.0] | 3.08 to 3.08 | 2, 4, 12, 13, 18, 27, 43, 51, 53, 59 |
| space_hulk_2 | orders | 3 | 3 | 5.0% [1.7, 13.7] | 0.23 to 0.23 | 29, 36, 58 |
| space_hulk_2 | squads | 0 | 7 | 11.7% [5.8, 22.2] | 0.00 to 0.58 | 10, 12, 20, 36, 44, 45, 53 |

All nine missions under `squads`, ten seeds each (liveness rows): space_hulk_6 10 of 10, space_hulk_4 8 of 10 (the same eight seeds as step 2), space_hulk_1 2 of 10, space_hulk_2 1 of 10 (was 0), space_hulk_5 1 of 10, every other mission 0 of 10; every game resolves, every row replays.

Toggle rows, one rule at a time on space_hulk_2 under squads (60 seeds): the blockade posting in stages under contact like defend gives the same 7 wins on the same seven seeds, and the reason is structural rather than luck: the stage opens at the first plan when nothing is in sight and never closes again (defend's staging works the same way, gating only a first plan made under contact), and on space_hulk_2 nothing is in sight when the order lands, so the staged variant never wrote a different task; the bot's gathering defend before the blockade gives 0 of 60 (mean end cycle 7.0 against 4.9: the two cycles the gathering costs are the cycles the blockade never gets to hold).

### The reading

1. The exit criterion holds: space_hulk_2 under squad orders went from 0 wins in 60 to 7 in 60 with the blockade, on the same seeds, nothing tuned. Against its own zero that is seven seeds flipped and none lost (McNemar p about 0.02). Against the individual-orders bot (3 in 60) the intervals overlap (5.8 to 22.2 against 1.7 to 13.7): six seeds flipped to wins and two the other way (seeds 29 and 58, which the per-marine spread wins and the blockade loses), a paired split of 6 to 2 that sixty seeds cannot call (p about 0.3). What the step establishes is that a single squad order can now blockade eleven entries at all; how much better than five marines walking alone is a question for the sweep.
2. space_hulk_1 is unchanged to the seed under both policies, as it should be: its objective resolves to the same threshold advance the issuer gave before, and the fan-out over one squad is the same order. The nine-mission rows moved only on space_hulk_2; space_hulk_4's eight winning seeds are the same eight because its two squads start quiet together and pass the gate on the same tick, so the fan-out is the order the second squad would have given itself that tick.
3. The win is a coverage win: on the quiet space_hulk_2 the blockade is complete at tick 80, the second cycle boundary, before the three reinforcement posts fill. Under fire the losses are the walk: seven marines' worth of corridor between the deployment rooms and the corner clusters, off the posts until the second or third boundary.
4. The two refinements above stand on the toggle rows: staging cannot bite on the blockade as the rule is written (kept off as the simpler rule, the switch stays; a staging that re-closed on later contact would be a new rule, not this one), and the gathering defend first is what kept space_hulk_2 at zero (0 of 60 with it, 7 without). Both are recorded here for Harry to overrule.
5. Nothing was tuned. regen.marine stays 3; no TUNING value changed.

Ships on /latest/ (a user-facing change on the prerelease line); v2.0.0-alpha.4 will freeze it with the next steps. Next: step 3 (the fixtures, deferred) or step 5 (the metered pause), Harry's call; step 6 still waits on the win-rate band.

## Stage 4 step 5: the metered command pause (2026-09-13)

Built on the "Command pause" section above and open questions 15 to 22 (all agreed); specs in pause_pool.spec (12 cases) and the client's pause.spec; the same sixty seeds re-scanned in two stages, raw output in docs/scans/2026-09-13-stage4-step5.txt.

### As built

- **The pool.** `GameEngine.pausePool` is a whole number of milliseconds. Its cap is 10 s plus 10 s per living sergeant (a sword sergeant counts) and it recharges 1 s per cycle plus 1 s per cycle per living sergeant, spread over the cycle's ticks by an integer remainder (exact for any cycle length), in tick step 1 after AP regeneration, silently. It starts full. The numbers are `TUNING.pausePool` (base, perSergeant, rechargeBase, rechargePerSergeant, seconds), the plan's proposal, and `?tuning=pausePool.base:30` overrides them like any other.
- **The bill.** At resume the client sends one `pauseSpent { ms }`, the wall-clock milliseconds the clock stood still, addressed to any living marine like a mission order (no lease, no task change); the engine subtracts it, clamps at zero, emits `pausePoolChanged`, and the command log carries it, so a seed plus the log replays the pool without the engine ever reading a clock (the state hash covers the pool and its remainder). A bill that is not a positive safe integer is refused. The bot never bills, so the scan measures the rules alone.
- **The pause.** Space opens the command pause when at least 1 s is left (below that the HUD flashes and nothing happens); the seconds run down on the HUD meter and the overlay bar, red in the last three; Space, the COMMAND button or the pool running out resumes, and each open costs at least a second (the open itself is priced, the one arrow wall-clock metering misses). Orders given in the pause reach their squad with the normal relay, so the pause buys thinking time and never reaction time. No lockout after a contact (question 17). A hidden tab bills the pause up to that moment and falls into the free pause. `?pause=free`, chosen before the mission, makes it unmetered (question 18).
- **Command points retired** (question 15): the d6 roll, the `cp` command, `cpChanged`, the P key and the roster's CP figure are gone; the HUD's one button reads COMMAND with the meter under it (the cap in 10 s segments, the recharge per cycle, the 1 s floor); the log format version is 2. The d6 drew a die at construction and at every cycle boundary, so every seed's dice stream shifted by one draw per cycle: the pinned seeds were re-pinned (marine_ai 26 to 43, quota_victory 29 to 12) and the scan is compared by rate and interval, not seed by seed.
- **Two refinements the passes forced**, recorded for Harry to overrule. A sergeant's death lowers the cap and the recharge at once but does not delete time already banked above the new cap: the surplus is kept and spent, never recharged (the advisor and the systems pass both called the instant clamp a cliff, and it would be a lockout at the moment the player most wants to think). And the pool is one for the whole force while the relay stays per squad (the pool is the player's attention, the relay is each sergeant's).

### The times (provisional: for review, as the idea asked)

| Living sergeants | Cap | Recharge | Refill from empty | Steady state: seconds paused per second of play |
|---|---|---|---|---|
| 0 | 10 s | 1 s per cycle (10 s of play) | 100 s of play | 0.10 (about 9% of wall clock) |
| 1 (a full squad today) | 20 s | 2 s per cycle | 100 s | 0.20 (about 17%) |
| 2 (two squads, both sergeants up) | 30 s | 3 s per cycle | 100 s | 0.30 (about 23%) |

The cap is the reserve (how many surprises in a row the player can answer); the recharge is the turn-based fraction the game settles into if the player pauses whenever he can, and it is the pause-scumming ceiling. Both are levers; the refill from empty is a constant 100 s of play at every sergeant count by construction (cap and recharge scale together). The four constants sit together in `TUNING.pausePool` and changing them moves no seed and no log version (the pool is not on the dice). Harry's review of these times is the open item of the step; until then they are provisional, and /latest/ carries them as such.

### The numbers (60 seeds, cycle cap 60, shipped tuning, 2026-09-13)

The removal was scanned in two stages. Stage (a) kept a phantom die drawn where the d6 used to be: all four 60-seed rows came back byte-identical to step 4 (the same winners to the seed), which proves the removal itself changed no behaviour. Stage (b) is the shipped build without the phantom, and its seeds are new games:

| Mission | Policy | Step 4 wins of 60 | Step 5 wins of 60 | Step 5 rate, 95% interval | Marines alive at the end (step 4 to step 5) | Step 5 winning seeds |
|---|---|---|---|---|---|---|
| space_hulk_1 | orders | 2 | 3 | 5.0% [1.7, 13.7] | 2.83 to 2.85 | 5, 13, 43 |
| space_hulk_1 | squads | 10 | 11 | 18.3% [10.6, 29.9] | 3.08 to 3.60 | 10, 21, 27, 28, 29, 32, 43, 51, 53, 54, 58 |
| space_hulk_2 | orders | 3 | 3 | 5.0% [1.7, 13.7] | 0.23 to 0.22 | 12, 36, 53 |
| space_hulk_2 | squads | 7 | 9 | 15.0% [8.1, 26.1] | 0.58 to 0.72 | 7, 10, 12, 13, 20, 23, 36, 44, 53 |

All nine missions under `squads`, ten seeds each (liveness rows): space_hulk_6 10 of 10, space_hulk_4 7 of 10 (was 8), space_hulk_5 2 of 10 (was 1), space_hulk_2 2 of 10 (was 1), space_hulk_1 1 of 10 (was 2), every other mission 0 of 10; every game resolves, every row replays.

### The reading

1. Nothing moved that sixty seeds can see. Every step 5 row sits inside its step 4 interval and every interval overlaps: the squad rows went 10 to 11 and 7 to 9, the orders rows 2 to 3 and 3 to 3, all within the seed-to-seed noise of a shifted dice stream. The bot never spent command points and never pauses, so this is the expected null result, and stage (a) is the proof that the code change itself is neutral.
2. The two-stage method is worth keeping: a rule change that moves the dice stream cannot be checked seed by seed, and "identical with a phantom draw, then compare rates without it" separates a broken engine from a moved stream in one extra scan.
3. The pause is unmeasured by design. The instrument has no player who pauses; what the meter does to a human's win rate is a playtest question (does the 1 s floor read; does the countdown pressure feel right; is 20 s enough to give five orders). The steady-state fractions in the times table are the numbers to argue about.
4. Nothing was tuned. regen.marine stays 3; no existing TUNING value changed; the pausePool group is new.

Ships on /latest/ (a user-facing change on the prerelease line); v2.0.0-alpha.4 will freeze it with the next steps. Next: step 3 (the fixtures, deferred) or step 6 (the sweep, needs the win-rate band), Harry's call; the times above want his review.
